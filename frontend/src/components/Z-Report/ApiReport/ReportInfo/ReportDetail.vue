<template>
	<div class="app-container">
		<el-card style="margin-bottom: 10px">
			...
			>
				<div
					class="report-seal-son"
					:style="{
						border: `solid 2px var(${reportStatus ? '--el-color-success' : '--el-color-danger'})`,
		...
		<el-card>
			...
			<div v-show="!isDebug">
				...
				<el-button class="ml10" type="primary" @click="searchDetail"> 查询 </el-button>
			</div>
		...
		<div>
			<el-drawer v-model="state.showDetailInfo" size="70%" append-to-body direction="ltr" destroy-on-close :with-header="true">
				<template #header>
					<span>
...
import { getMethodColor, getStatusTag, stepTypeEnum } from '/@/utils/case';
...
const getStatisticsDataByDebug = (step_results) => {
	...
	step_results.forEach((e) => {
		if ([stepTypeEnum.Api].includes(e.step_type)) {
...
const getList = () => {
	...
	useReportApi()
		.getReportDetail(state.listQuery)
		.then((res) => {
...
const getStatistics = () => {
	useReportApi()
		.getReportStatistics(state.statQuery)
		.then((res) => {
