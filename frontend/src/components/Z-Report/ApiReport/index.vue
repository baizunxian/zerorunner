<template>
	<div class="report-container">
		<el-tabs v-model="state.activeName" class="demo-tabs">
			<template v-if="[stepTypeEnum.Api, stepTypeEnum.Sql, stepTypeEnum.Script].includes(state.step_type)">
				<el-tab-pane name="ResponseInfo">
					<template #label>
						<strong>响应信息</strong>
					</template>
					<!--					<ResponseInfo :data="state.responseInfo" :stat="state.stat" ref="responseInfoRef"></ResponseInfo>-->
					<ApiResponseInfo v-if="state.step_type === stepTypeEnum.Api" :data="state.responseInfo" :stat="state.stat" ref="responseInfoRef" />
					<SqlResponseInfo v-if="state.step_type === stepTypeEnum.Sql" :data="state.responseInfo" :stat="state.stat" ref="responseInfoRef" />
					<PyScriptResponseInfo v-if="state.step_type === stepTypeEnum.Script" :data="state.responseInfo" :stat="state.stat" ref="responseInfoRef" />
				</el-tab-pane>
				...
				<el-tab-pane name="RequestInfo">
					...
					<ApiRequestInfo v-if="state.step_type === stepTypeEnum.Api" :data="state.requestInfo" />
					<SqlRequestInfo v-if="state.step_type === stepTypeEnum.Sql" :data="state.requestInfo" />
					<PyScriptRequestInfo v-if="state.step_type === stepTypeEnum.Script" :data="state.requestInfo" />
			...
			<el-tab-pane name="preHookData">
				...
				<ReportHooks :setup-hook-results="state.setup_hook_results" :teardown-hook-results="state.teardown_hook_results"> </ReportHooks>
			</el-tab-pane>
...
import ApiResponseInfo from './components/response-info/ApiResponseInfo.vue';
import ApiRequestInfo from './components/request-info/ApiRequestInfo.vue';
import SqlResponseInfo from './components/response-info/SqlResponseInfo.vue';
import SqlRequestInfo from './components/request-info/SqlRequestInfo.vue';
import PyScriptResponseInfo from './components/response-info/PyScriptResponseInfo.vue';
import PyScriptRequestInfo from './components/request-info/PyScriptRequestInfo.vue';
...
const initData = () => {
	...
	if ([stepTypeEnum.Api, stepTypeEnum.Sql, stepTypeEnum.Script].includes(state.step_type)) {
		state.stat = step_result.session_data.stat;
		state.responseInfo = step_result.session_data?.response;
		state.requestInfo = step_result.session_data?.request;
