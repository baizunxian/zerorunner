<template>
	<div @click.stop="">
		<div class="sql-request-box">
			<el-form ref="formRef" label-position="right" label-width="75px" :model="state.requestData">
				<el-row :gutter="24">
					...
					<template v-if="state.requestData.use_type == 'source'">
						<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb10">
							<el-form-item label="运行环境" prop="project_id">
								<el-select size="small" v-model="state.requestData.env_id" placeholder="运行环境" filterable style="width: 100%" @change="selectEnv">
						...
						<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb10">
							<el-form-item label="数据源名称" prop="module_id">
								<el-select size="small" v-model="state.requestData.source_id" placeholder="请选择" filterable style="width: 100%">
									<el-option v-for="source in state.sourceList" :key="source.id + source.name" :label="source.name" :value="source.data_source_id">
					...
					<template v-if="state.requestData.use_type == 'custom'">
						<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb10">
							<el-form-item label="数据源类型" prop="source_type" :rules="[{ required: true, message: '请选择数据源类型', trigger: 'blur' }]">
						...
						<el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12" class="mb10">
							<el-form-item label="地址" prop="host" :rules="[{ required: true, message: '请输入地址', trigger: 'blur' }]">
						...
						<el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12" class="mb10">
							<el-form-item label="端口" prop="port" :rules="[{ required: true, message: '请输入端口', trigger: 'blur' }]">
								<el-input type="number" v-model="state.requestData.port" placeholder="请输入端口" clearable></el-input>
							</el-form-item>
						</el-col>
...
const selectEnv = (env_id: any) => {
	...
	if (state.requestData) {
		state.requestData.source_id = null;
	}
};
...
const setData = (data: any) => {
	...
	state.requestData.source_type = state.requestData?.source_type ?? 'mysql';
	state.requestData.use_type = state.requestData?.use_type ?? 'source';
};
