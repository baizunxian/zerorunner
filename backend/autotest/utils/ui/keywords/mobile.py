...
from autotest.utils.ui.web.locator import locating_element
...
def check(step,g):
    ...
    element_location = locating_element(element, g)
    if '#' in element:
        e_name = step['keyword']
    else:
        e_name = element
...
def input(step, g):
    ...
    if isinstance(data['text'], tuple):
        ...
        element_location.send_keys(data['text'])
    g.driver.get_screenshot_as_file('输入完成.png')
...
def line(step):
    ...
    assert isinstance(element, list) and len(
        element) > 1, '坐标格式或数量不对，正确格式如：258,756|540,1032'
    postions = []
    for _e in element:
        _e = _e.replace('，', ',')
        p = _e.split(',')
...
def flick_element(step):
    ...
    assert isinstance(element, list) and len(
        element) == 2, '坐标格式或数量不对，正确格式如：elment|200,300'
    _e = eval(element[0])
