...
from autotest.corelibs import g, logger
...
def create_celery():
    ...
    class ContextTask(Task, ABC):
        ...
        def __call__(self, *args, **kwargs):
            ...
            g.redis = IOLoop.current().run_sync(lambda: init_redis_pool())
            _task_stack.push(self)
            self.push_request(args=args, kwargs=kwargs)
            try:
                if asyncio.iscoroutinefunction(self.run):
                    return IOLoop.current().run_sync(lambda: self.run(*args, **kwargs))
                else:
    ...
    _celery_: Celery = Celery(task_cls=ContextTask)
...
# celery -A autotest.celery_worker.celery worker --pool=solo -l INFO  windows 启动，只能单线程
# celery -A autotest.celery_worker.celery beat  -l INFO  启动节拍器，定时任务需要
# celery -A autotest.celery_worker.celery beat -S autotest.celery_worker.scheduler.schedulers:DatabaseScheduler -l INFO
# celery -A autotest.celery_worker.celery beat -S tools.corelibs.scheduler.schedulers:DatabaseScheduler -l INFO
# 当不是作为worker跑的时候,就不需要初始化celery_worker
# PromiseProxy 用于lazy load, 如果不操作celery_worker, 就不会有初始化的开销

# celery worker -A autotest.celery_worker.celery --loglevel=INFO --pool=solo
