...
def get_case(case_id, result_field):
    ...
    :return:
    """
    details = run_test_by_id(case_id)
...
def db_select(host, port, user, password, database, sql, decrypt=False):
    """
    数据库查询
    :param host: 数据库地址
    :param port: 端口号
    ...
    :return: list
    """
    db = DB(host, port, user, password, database, decrypt)
    return db.select(sql)


def db_execute(host, port, user, password, database, sql, decrypt=False):
    """
    执行sql语句
    :param host: 数据库地址
    :param port: 端口号
...
def hive_select(host, port, database, sql, user=None, password=None):
    """
    数据库查询
    :param host: 数据库地址
    :param port: 端口号
    ...
    :return: list
    """
    h = Hive(host, port, database, user, password)
    return h.select(sql)


def presto_select(host, port, sql, user=None, password=None):
    p = PrestoDB(host=host, port=port, user=user, password=password)
    return p.select(sql)
