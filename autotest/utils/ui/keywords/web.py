...
# from autotest.utils.ui.globals import g
# from sweetest.elements import e
# from autotest.utils.ui.keywords.windows import w
...
class Common:
    ...
    @classmethod
    def current_url(cls, data, output, g):
        # logger.info('DATA:%s' % repr(data['text']))
...
def check(step):
    ...
    element = step['element']
    element_location = locating(element)
    if '#' in element:
...
def input(step, g):
    data = step['data']
    element = step['element']
    element_location = locating(element, g.driver)
...
def switch_to_frame(step, g):
    data = step['data']
    element = step['element']
    element_location = locating(element, g.driver, 'XPATH')
    g.driver.implicitly_wait(10)
    g.driver.switch_to.frame(element_location)
...
def click(step, g):
    ...
    if isinstance(element, dict):
        element_location = locating(element, g, 'CLICK')
        if element_location:
            try:
