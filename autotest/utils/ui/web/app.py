...
from autotest.utils.ui.keywords.windows import Windows
...
class App:
    ...
    def __init__(self, setting):
        ...
        self.browserName = setting.get('browserName', 'chrome')
        self.headless = setting.pop('headless', False)
        # snapshot = setting.pop('snapshot', False)
        # self.executable_path = setting.pop('executable_path', False)
        self.executable_path = None
        self.chrome_driver = os.path.abspath(r"/data/project/selenium/chromedriver")
        ...
        if self.browserName.lower() == 'ie':
            ...
            if self.executable_path:
                ...
                chrome_capabilities = {
                    ...
                    "webdriver.chrome.driver": self.chrome_driver
                }
    ...
    def title(self, data, output):
        logger.debug(f'DATA:{repr(data["text"])}')
        logger.debug(f'REAL:{repr(self.driver.title)}')
        # try:
    ...
    def current_url(self, data, output):
        logger.debug(f'DATA:{repr(data["text"])}')
        logger.debug(f'REAL:{repr(self.driver.current_url)}')
        try:
            if data['text'].startswith('*'):
    ...
    def open(self, step):
        ...
        if isinstance(step['data'], dict):
            ...
            if cookie:
                ...
                co = self.driver.get_cookie(json.loads(cookie).get('name', ''))
                logger.debug(f'cookie is add: {co}')
        self.driver.get(url)
        sleep(0.5)
    ...
    def check(self, step):
        ...
        if by in ('title', 'current_url'):
            ...
            for key in data:
                ...
                logger.debug(f'DATA:{repr(expected)}')
                logger.debug(f'REAL:{repr(real)}')
                try:
    ...
    def click(self, step):
        ...
        element = step['element']
        location = ''
        # for element in step.get('element'):
    ...
    def switch_to_frame(self, step):
        data = step['data']
        element = step['element']
        element_location = self.locat(element)
